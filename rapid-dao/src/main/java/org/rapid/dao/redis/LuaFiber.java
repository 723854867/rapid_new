...
public class LuaFiber {
	...
	@Resource
	private Redis redis;
	private Map<ILuaCmd, Script> scripts = new ConcurrentHashMap<ILuaCmd, Script>();		
	
	public void addScript(ILuaCmd cmd, byte[] content) {
	...
	<T> T invoke(ILuaCmd cmd, Object... params) {
		...
		return redis.invoke(new Callback<Jedis, T>() {
			...
			public T invoke(Jedis jedis) {
				if (script.stored) {
					try {
						return (T) jedis.evalsha(SerializeUtil.RedisUtil.encode(script.sha1Key), cmd.keyNum(), arr);
					} catch (JedisNoScriptException e) {
						logger.warn("redis lua 脚本 - {} 缓存未命中,直接执行脚本！", cmd);
					}
				}
				T object = (T) jedis.eval(script.content, cmd.keyNum(), arr);
				script.stored = true;
				return object;
			}
		});
	...
	private class Script {
		...
		private Script(byte[] content) {
			...
			this.content = SerializeUtil.RedisUtil.encode(new String(content));
		}
	}
}
